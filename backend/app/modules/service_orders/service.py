...
class ServiceOrderService:
    ...
    def list_orders(self, tenant_id: uuid.UUID | None = None) -> list[ServiceOrder]:
        return self.repo.list_all(tenant_id)

    def get_order(self, order_id: uuid.UUID, tenant_id: uuid.UUID | None = None) -> ServiceOrder:
    ...
    def create_order(self, data: ServiceOrderCreate, tenant_id: uuid.UUID | None = None) -> ServiceOrder:
        order = ServiceOrder(
            tenant_id=tenant_id,
    ...
    def update_order(self, order_id: uuid.UUID, data: ServiceOrderUpdate, tenant_id: uuid.UUID | None = None) -> ServiceOrder:
    ...
    def change_status(self, order_id: uuid.UUID, data: StatusUpdate, tenant_id: uuid.UUID | None = None) -> ServiceOrder:
        order = self.get_order(order_id, tenant_id)
    ...
    def assign_mechanic(self, order_id: uuid.UUID, data: AssignMechanic, tenant_id: uuid.UUID | None = None) -> ServiceOrder:
        order = self.get_order(order_id, tenant_id)
